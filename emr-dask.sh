...
# fixed python version, only 3.7 works
...
# FIXES:
# installing libraries one by one instead of all at once, otherwise conda get stuck for some reason
# fixed pyarrow to version 3.0.0
# fixed tornado to 6.1, tornado 5 does not work
# fixed s3fs to 0.3.0
...
conda install -c conda-forge -y -q dask=2021.04.0
conda install -c conda-forge -y -q dask-ml
conda install -c conda-forge -y -q distributed=2021.04.0
$EXTRA_CONDA_PACKAGES
